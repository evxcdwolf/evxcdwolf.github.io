...
export default function App() {
  ...
  useEffect(() => {
    ...
    setStickerConfig((prev) => {
      ...
      const isValidImage = mergedPool.includes(prev.imageUrl) || prev.imageUrl?.startsWith('data:') || prev.imageUrl?.startsWith('http');
      const chosenSticker = (prev.randomOnReload || !isValidImage) && mergedPool.length > 0
        ? mergedPool[Math.floor(Math.random() * mergedPool.length)]
        : (isValidImage ? prev.imageUrl : mergedPool[0] || '');
      return {
        ...prev,
