...
import { DYNAMIC_STICKER_POOL } from '../data/defaultProfile';
import {
  Sparkles,
  Shuffle,
  Sliders,
  ...
  return (
    <div className="relative min-h-screen flex flex-col justify-center items-center px-3 sm:px-6 py-6 sm:py-10 z-10 w-full max-w-full overflow-x-hidden">
      ...
      >
        ...
        {/* TOP BUTTON ROW (Steam, x.com, Proton Mail) */}
            ...
            {stickerConfig.enabled && (
              ...
              >
                <img
                  ...
                  onError={() => {
                    if (DYNAMIC_STICKER_POOL && DYNAMIC_STICKER_POOL.length > 0) {
                      const randomFallback = DYNAMIC_STICKER_POOL[Math.floor(Math.random() * DYNAMIC_STICKER_POOL.length)];
                      onUpdateStickerConfig({
                        ...stickerConfig,
